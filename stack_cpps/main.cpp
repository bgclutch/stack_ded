...
int main(void)
{
    ...
    for(size_t i = stack_data.size; i >= 1; i--)
    {
        ...
        REVERSE_ASSERT(pop_result && "pop result error";)
    }

    TEMP_VAR(Error_Codes dtor_result = ) dtor_stack(&stack_data);
    REVERSE_ASSERT(dtor_result && "dtor result error";)
