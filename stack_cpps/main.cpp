...
int main(void)
{
    ...
    REVERSE_ASSERT(ctor_result && "stack constructor error") TEMP_VAR(;) 

    for(size_t i = 0; i < sizeof(test) / sizeof(StackElem_t); i++)
    {
        elem = test[i];
        TEMP_VAR(Error_Codes push_result =) stack_push(&stack_data, elem);
        REVERSE_ASSERT(push_result && "push result error") TEMP_VAR(;) 
    }
    ...
    for(size_t i = stack_data.size; i >= 1; i--)
    {
        ...
        REVERSE_ASSERT(pop_result && "pop result error") TEMP_VAR(;) 
    }

    TEMP_VAR(Error_Codes dtor_result = ) dtor_stack(&stack_data);
    REVERSE_ASSERT(dtor_result && "dtor result error") TEMP_VAR(;) 
